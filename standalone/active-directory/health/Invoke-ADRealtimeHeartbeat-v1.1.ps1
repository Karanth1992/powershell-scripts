...
.DESCRIPTION
    ...
    - `C:\Temp\Reports\ADRealtimeHeartbeat`
...
.PARAMETER OutputFolder
    ...
    Defaults to `C:\Temp\Reports\ADRealtimeHeartbeat`.
...
.EXAMPLE
    .\Invoke-ADRealtimeHeartbeat-v1.1.ps1 -SendEmail -To 'adops@example.com' -From 'ad-monitor@example.com' -SmtpServer 'smtp.example.com'
...
.EXAMPLE
    .\Invoke-ADRealtimeHeartbeat-v1.1.ps1 -ComputerName 'DC01.corp.contoso.com','DC02.corp.contoso.com' -SendSlack -SlackWebhookUrl 'https://hooks.slack.com/services/...'

    Tests two explicit domain controllers and sends Slack alerts for failed
    conditions.

.EXAMPLE
    .\Invoke-ADRealtimeHeartbeat-v1.1.ps1 -DomainName 'corp.contoso.com' -AlertOnWarning -SendEmail -To 'adops@example.com' -From 'ad-monitor@example.com' -SmtpServer 'smtp.example.com'

    Tests all domain controllers in one domain and sends alert messages for
    both warning and failed conditions.

.EXAMPLE
    .\Invoke-ADRealtimeHeartbeat-v1.1.ps1 -SendEmail -To 'adops@example.com' -From 'ad-monitor@example.com' -SmtpServer 'smtp.example.com' -WhatIf
...
param(
    ...
    [string]$OutputFolder = 'C:\Temp\Reports\ADRealtimeHeartbeat',
