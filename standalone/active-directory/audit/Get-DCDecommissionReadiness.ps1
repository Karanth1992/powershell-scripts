...
.PARAMETER OutputPath
    Folder to write the HTML report. Default: C:\Logs\DCDecomm.

.PARAMETER Credential
    Alternate credentials if running from a non-domain-joined machine.

.EXAMPLE
    .\Get-DCDecommissionReadiness.ps1 -DCName "DC01.corp.contoso.com"

.EXAMPLE
    .\Get-DCDecommissionReadiness.ps1 -DCName "DC01.corp.contoso.com" -DaysBack 14 -OutputPath "D:\Reports"
...
param(
    ...
    [string]$OutputPath = "C:\Logs\DCDecomm",
...
if (-not (Test-Connection -ComputerName $DCName -Count 1 -Quiet)) {
    Write-Host "  [FATAL] Cannot reach $DCName. Aborting." -ForegroundColor Red
    exit 1
}
...
$netlogon = Get-SmbShare -CimSession $DCName -Name NETLOGON -ErrorAction SilentlyContinue
